...
const Submissions = () => {
    ...
    useEffect(() => {
        axiosInstance.get('submissions/').then((res) => {
            res.data.results.forEach(submission => {
                if(submission.question_id_fk === 1){
                    setQue1([...que1, { id: submission.question_id_fk, time: submission.submission_time.substring(12, 16), progress: submission.accuracy }]);
                }
            });
        })
    }, [setQue1, setQue2, setQue3, setQue4, setQue5, setQue6]);
